...
## Tối ưu chi phí OpenAI trong V27

Bản V27 thêm chế độ **Tiết kiệm token AI** mặc định bật sẵn.

Cơ chế mới:

- Lệnh trực tiếp rõ ràng như `long bitcoin vốn 10u x10`, `mua BTC spot 10u`, `bán hết BTC spot` sẽ được parser nội bộ xử lý trước, không gọi OpenAI.
- Prompt DCA đơn giản như `mua bitcoin spot 10 usdt/1h` sẽ được scheduler + parser xử lý, không gọi AI ở mỗi vòng.
- Bot chỉ gọi AI khi prompt thật sự cần phân tích phức tạp, ví dụ có điều kiện chỉ báo, nhiều rule hoặc câu lệnh mơ hồ.
- Snapshot gửi cho AI được rút gọn, không gửi toàn bộ dữ liệu Bybit/raw klines/wallet thừa.
- Bybit Skill context gửi cho AI được giới hạn bằng biến `BYBIT_SKILL_CONTEXT_MAX_CHARS`, mặc định 1800 ký tự.
- Model mặc định đổi sang `gpt-4o-mini` để giảm chi phí thử nghiệm.

Khuyến nghị khi test:

```env
OPENAI_MODEL=gpt-4o-mini
BYBIT_SKILL_CONTEXT_MAX_CHARS=1200
BOT_INTERVAL_SECONDS=60
```

Trong dashboard, có thể tắt/bật mục **Tiết kiệm token AI** tại phần Cài đặt API & Rủi ro.
